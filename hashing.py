...
class ChainingValue(list):
    def __init__(self, x):
        if isinstance(x, str):
            l = [int(x[8*i:8*(i+1)], 16) for i in range(8)]
            super().__init__(l)
        else:
            super().__init__(x)

    def __add__(self, x):
        return ChainingValue([(self[i] + x[i]) & 0xffffffff for i in range(8)])
    ...
    def hex(self):
        return "".join("{0:08x}".format(x) for x in self)
...
class sha256:
    ...
    def _finalize(self):
        ...
        bit_count = self.count * 8
        for i in range(8):
            self.data[63 - i] = (bit_count >> (8 * i)) & 0xff
        self.compress()
...
def get_padding(length_in_bytes):
    pad = b'\x80'
    pad += b'\x00' * ((56 - (length_in_bytes + 1) % 64) % 64)
    bit_len = length_in_bytes * 8
    pad += struct.pack('>Q', bit_len)
    return pad


# Paramètres du challenge
original_tag = "165daddb7e8019461e69bcda99511e2c64a10c3d79c8aa025371e655affbef27"
key_len = 16
...
padding_bytes = get_padding(key_len + len(m_original))
padding_hex = "".join([f"\\x{b:02x}" for b in padding_bytes])

forge = sha256(IV=original_tag)
forge.count = key_len + len(m_original) + len(padding_bytes)
forge.update(suffixe.encode())
nouveau_tag = forge.hexdigest()

print("=== NOUVEAU TAG MAC ===")
print(nouveau_tag)
print("\n=== CODE RML A COPIER DANS LE JEU ===")
print(f"{m_original}{padding_hex}{suffixe}")